---
layout: post
title: "Autolayout小结(二)"
date: 2015-02-02 15:52:55 +0800
comments: true
categories: 能工巧匠集
---
在[Autolayout小结(一)](http://blog.cnbluebox.com/blog/2014/12/13/autolayout1/)中介绍了在Autolayout学习中一些基本的注意点，本文会针对一些布局上常见的问题进行分析。
> 1. 如何自动适应cell的高度
...
> 2. 要保证垂直方向上的Constraints理论上可以计算出cell的高度，另：最好调低其中一个Constraint的优先级，避免约束发生冲突
...
在strackoverflow有回答上说明了为什么`UITableViewCell`等部分系统控件上添加Constraints会crash, 即`UITableViewCell`等部分控件的`layoutSubviews`里面没有执行`[super layoutSubviews]`。 
